...

{
  ...
  systemd.services."docker-pihole" = {
    ...
    after = [
      "docker-network-whoami_backend.service"
    ];
    requires = [
      "docker-network-whoami_backend.service"
    ];
  ...
  # Networks
  systemd.services."docker-network-whoami_backend" = {
    path = [ pkgs.docker ];
    serviceConfig = {
      Type = "oneshot";
      RemainAfterExit = true;
      ExecStop = "docker network rm -f whoami_backend";
    };
    script = ''
      docker network inspect whoami_backend || docker network create whoami_backend --subnet=172.31.0.0/16
    '';
    partOf = [ "docker-compose-whoami-root.target" ];
    wantedBy = [ "docker-compose-whoami-root.target" ];
  };
